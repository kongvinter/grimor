...
//    
 ...
 document.addEventListener('DOMContentLoaded', () => {
    ...
    const inpHeight = document.getElementById('height');
    const inpSeed = document.getElementById('seed');
    const inpFrag = document.getElementById('fragmentation');
    ...
    function generateMap() {
      ...
      // Configura e gera o mapa
      const generator = new window.MapGenerator({
        width, height, seed,
        palette: window.palettes.default,
    ...
    // Valor de seed aleatório se vazio
    if (!inpSeed.value) inpSeed.value = Math.floor(Math.random() * 1e9);
